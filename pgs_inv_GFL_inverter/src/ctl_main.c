...
void ctl_step_dc_bus_voltage_ctrl(void)
{
    ...
    ctrl_gt power_limit_pu = float2ctrl(GFL_DCBUS_POWER_LIMIT_PU);
    ...
    ctrl_gt voltage_error_pu = dc_bus_voltage_ref_ramp_pu - inv_ctrl.filter_udc.out;
    ctrl_gt absorbed_power_ref_pu = ctl_step_pid_ser(&dc_bus_voltage_ctrl, voltage_error_pu);

    // When the rectification command reaches its upper limit, back-calculate
    // the integrator state so it cannot keep accumulating unavailable power.
    // This preserves the faster charging command while reducing the DC-bus
    // overshoot after the voltage approaches its target.
    if (absorbed_power_ref_pu >= power_limit_pu && voltage_error_pu > 0)
    {
        ctl_pid_clamping_correction_using_real_output(&dc_bus_voltage_ctrl, power_limit_pu);
    }

    dc_bus_power_ref_pu = -absorbed_power_ref_pu;
